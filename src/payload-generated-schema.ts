...
export const library = pgTable(
  ...
  (columns) => ({
    library_created_by_idx: index("library_created_by_idx").on(
      columns.createdBy
    ),
...
export const library_rels = pgTable(
  "library_rels",
  {
    id: serial("id").primaryKey(),
    order: integer("order"),
  ...
  (columns) => ({
    order: index("library_rels_order_idx").on(columns.order),
    parentIdx: index("library_rels_parent_idx").on(columns.parent),
    pathIdx: index("library_rels_path_idx").on(columns.path),
    library_rels_games_id_idx: index("library_rels_games_id_idx").on(
      columns.gamesID
    ),
    parentFk: foreignKey({
      columns: [columns["parent"]],
      foreignColumns: [library.id],
      name: "library_rels_parent_fk",
    }).onDelete("cascade"),
    gamesIdFk: foreignKey({
      columns: [columns["gamesID"]],
      foreignColumns: [games.id],
      name: "library_rels_games_fk",
    }).onDelete("cascade"),
  })
);
...
export const relations_library_rels = relations(library_rels, ({ one }) => ({
  parent: one(library, {
    fields: [library_rels.parent],
    references: [library.id],
    relationName: "_rels",
  }),
  gamesID: one(games, {
    fields: [library_rels.gamesID],
    references: [games.id],
    relationName: "games",
  }),
}));
export const relations_library = relations(library, ({ one, many }) => ({
  createdBy: one(users, {
    fields: [library.createdBy],
    references: [users.id],
...
type DatabaseSchema = {
  ...
  library_rels: typeof library_rels;
  notes: typeof notes;
  ...
  relations_library_rels: typeof relations_library_rels;
